...
function excelTimeToMin(v) {
  ...
  return Math.round(num * 24 * 60 * 10) / 10
}
...
export async function parseTiemposCarga(file, fecha, user, turno, momento) {
  ...
  try {
    ...
    const headerRow = aoa.findIndex((row) => row && String(row[0] ?? '').toUpperCase().includes('CARGA'))
    if (headerRow < 0) throw new Error('No se encontró fila de encabezados.')
    ...
    for (let r = headerRow + 1; r < aoa.length; r++) {
      ...
      if (!row) continue
      const ruta = String(row[0] ?? '').trim()
      if (!ruta || !ruta.match(/^[A-Z]{2}\d+/i)) continue
      ...
      records.push({
        ...
        momento: momento ?? null,
        ruta,
        cajas: cleanNumericString(row[1]),
        cj_en_tarimas: cleanNumericString(row[2]),
        cj_reales: cleanNumericString(row[3]),
        inicio_carga: excelTimeToHHMM(row[5]),
        fin_carga: excelTimeToHHMM(row[6]),
        tiempo_carga_min: excelTimeToMin(row[7]),
        inicio_embarque: excelTimeToHHMM(row[8]),
        fin_embarque: excelTimeToHHMM(row[9]),
        tiempo_embarque_min: excelTimeToMin(row[10]),
        cj_x_min: cleanNumericString(row[11]),
        equipo: row[12] != null ? String(row[12]).trim() : null,
        tetra: String(row[13] ?? '').toUpperCase().trim() === 'SI',
      })
    }

    await deleteAndInsert('tiempos_carga', { fecha, turno, momento }, records, BATCH)
