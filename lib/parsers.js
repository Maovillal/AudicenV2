...
export async function parseConteoFisico(file, fechaUsuario, user, turno, momento) {
  ...
  try {
    ...
    const effectiveFecha = fechaUsuario

    // Apertura digital = inventario_liquido del día siguiente, T1, inicio
    const nextDay = new Date(effectiveFecha + 'T12:00:00')
    ...
    for (const ir of invRows) {
      ...
      if (!k) continue
      const total =
        (cleanNumericString(ir.stock_libre) ?? 0) +
        (cleanNumericString(ir.stock_bloqueado) ?? 0) +
        (cleanNumericString(ir.stock_calidad) ?? 0)
      invBySku.set(k, total)
    }
    ...
    const COL = {
      ...
      total_tarimas: 15,
      cajas_en_tarimas: 16,
      total_fisico: 20,        // columna U — total físico bruto
      merma_operativa: 30,     // columna AE
      merma_total: 31,         // columna AF
      merma_dora: 32,          // columna AG
      total_picos: 33,         // columna AH
      total_fisico_real: 34,   // columna AI — real en piso
    }
    ...
    for (let r = 2; r < aoa.length; r++) {
      ...
      const mermaTot = cleanNumericString(row[COL.merma_total])
      const totalFisReal = cleanNumericString(row[COL.total_fisico_real])
      ...
      const diferencia = totalFisReal != null && totalSist != null
        ? totalFisReal - totalSist
        : totalFisReal != null ? totalFisReal : null
      ...
      records.push({
        ...
        merma_total: mermaTot,
        total_fisico_real: totalFisReal,
...
export async function parseConciliacionEnvase(file, fecha, user, turno, momento) {
  ...
  try {
    ...
    const sheetName = findSheetByFecha(wbMeta, fecha)
    ...
    for (let r = aoa.length - 1; r >= 0; r--) {
      ...
      for (let c = 0; c < row.length; c++) {
        if (normalizeCellText(row[c]) === 'finsap') {
    ...
    if (finSapVal == null) {
      return { success: false, registros: 0, error: 'No se encontró el valor "Fin SAP" en el archivo.' }
    }
...
/**
 * Parser para "Indicador Nivel de Servicio Rutas" (.xlsx).
 *
 * Una pestaña por día con nombre dd.mm.yyyy. Cada fila es una ruta
 * (RK1601-RK1620 típicamente). Las columnas NS y MINUTOS son fórmulas;
 * leemos con raw:false para obtener el valor calculado y no la fórmula.
 *
 * Schema destino (tabla nivel_servicio):
 *   fecha, ruta, tiempo_estimado, hora_inicio, hora_termino, minutos, observaciones
 *
 * @param {File} file
 * @param {string} fecha
 * @param {import('@supabase/supabase-js').User | null | undefined} [user]
 * @returns {Promise<ParseResult>}
 */
export async function parseNivelServicio(file, fecha, user, turno, momento) {
  if (!file || !fecha) return { success: false, registros: 0, error: 'Falta archivo o fecha.' }
  try {
    const ab = await file.arrayBuffer()
    const wb = XLSX.read(ab, { type: 'array', cellDates: false })
    ...
    const sheetName = wb.SheetNames.find((n) => n.trim().startsWith(target))
    if (!sheetName) {
      const disponibles = wb.SheetNames.slice(-10).join(', ')
      throw new Error(
        `No se encontró pestaña para ${d}/${m}/${y}. ` +
        `Últimas pestañas disponibles: ${disponibles}. ` +
        `Verifica que la fecha seleccionada coincida con una pestaña del archivo.`
      )
    }

    // raw:false devuelve texto formateado (ej. "19:11", "378") en lugar de
    // fracciones/nulls — necesario porque NS/MINUTOS son fórmulas y las
    // celdas de tiempo no tienen caché de valor raw.
    const aoa = XLSX.utils.sheet_to_json(wb.Sheets[sheetName], { header: 1, defval: null, raw: false })

    // Buscar fila de headers — la que contenga "RUTA" y otras columnas clave.
    const headerRow = aoa.findIndex((row) =>
      row && row.some((cell) => /^\s*ruta\s*$/i.test(String(cell ?? '')))
    )
    if (headerRow < 0) throw new Error(`No se encontró fila de encabezados en pestaña "${sheetName}".`)
    ...
    const iRuta          = findCol(['RUTA'])
    const iEstimado      = findCol(['ESTIMADO', 'ESTIMA'])
    const iInicio        = findCol(['INICIO'])
    const iTermino       = findCol(['TERMINO', 'TÉRMINO', 'FIN'])
    const iMinutos       = findCol(['MINUTOS', 'MIN'])
    const iObservaciones = findCol(['OBSERV', 'NOTAS'])

    if (iRuta < 0) throw new Error(`Header "RUTA" no encontrado en pestaña "${sheetName}".`)
    ...
    for (let r = headerRow + 1; r < aoa.length; r++) {
      const row = aoa[r]
      if (!row) continue
      const ruta = String(row[iRuta] ?? '').trim()
      if (!ruta) continue
      if (SKIP_WORDS.some((w) => ruta.toLowerCase().includes(w))) continue
      ...
      records.push({
        fecha,
        ruta,
        tiempo_estimado: iEstimado >= 0 ? cleanNumericString(row[iEstimado]) : null,
        hora_inicio:     iInicio   >= 0 ? excelTimeToHHMM(row[iInicio])      : null,
        hora_termino:    iTermino  >= 0 ? excelTimeToHHMM(row[iTermino])     : null,
        minutos:         iMinutos  >= 0 ? excelTimeToMin(row[iMinutos])      : null,
        observaciones:   iObservaciones >= 0 && row[iObservaciones] != null
                          ? String(row[iObservaciones]).trim() || null : null,
      })
    }

    if (records.length === 0) throw new Error(`No se encontraron rutas en pestaña "${sheetName}".`)

    await deleteAndInsert('nivel_servicio', { fecha }, records, BATCH)
    await logUpload('nivel_servicio', records.length, fecha, user, turno, momento)
    return { success: true, registros: records.length, fechaDatos: fecha }
  } catch (e) {
    return { success: false, registros: 0, error: e?.message || String(e) }
  }
...
export async function runParser(tipo, file, fecha, user, turno, momento) {
  switch (tipo) {
    ...
    case 'nivel_servicio':
      return parseNivelServicio(file, fecha, user, turno, momento)
    case 'ingreso_envase':
