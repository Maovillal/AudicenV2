...
export async function parseConteoFisico(file, fechaUsuario, user, turno, momento) {
  ...
  try {
    ...
    for (const ir of invRows) {
      ...
      if (!k) continue
      // Sobrescribir en lugar de acumular — un solo valor por SKU
      invBySku.set(k, cleanNumericString(ir.stock_libre) ?? 0)
    }
    ...
    const COL = {
      ...
      merma_total: 31,        // columna AF — merma total del SKU
    ...
    for (let r = 2; r < aoa.length; r++) {
      ...
      // total real en piso = total físico bruto − merma total
      const totalFisReal = totalFis != null ? (totalFis - (mermaTot ?? 0)) : null
      ...
      let diferencia = null
      if (totalFisReal != null && totalSist != null) {
        diferencia = totalFisReal - totalSist
      } else if (totalFisReal != null && totalSist == null) {
        diferencia = totalFisReal
      }
