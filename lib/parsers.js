 ...
 */
async function logUpload(tipoArchivo, registros, fechaDatos, user) {
 ...
 */
export async function parseInventarioLiquido(file, fecha, user) {
  if (!file || !fecha) {
  ...
  try {
    ...
    const momento = parseMomentoFromFileName(file.name)
    const turno = parseTurnoFromFileName(file.name)
    ...
    await logUpload('inventario_liquido', records.length, fecha, user)
 ...
 * @returns {Promise<ParseResult>}
 */
export async function parseSalidasRutas(file, fecha, user) {
  if (!file || !fecha) {
    return { success: false, registros: 0, error: 'Falta archivo o fecha.' }
  }
  try {
    ...
    await deleteAndInsert('salidas_rutas', { fecha }, records, BATCH)
    await logUpload('salidas_rutas', records.length, fecha, user)
    return { success: true, registros: records.length, fechaDatos: fecha }
  } catch (e) {
    return { success: false, registros: 0, error: e?.message || String(e) }
  }
 ...
 * @returns {Promise<ParseResult>}
 */
export async function parseConteoFisico(file, fechaUsuario, user) {
  ...
  try {
    ...
    await logUpload('conteo_fisico', records.length, effectiveFecha, user)
 ...
 * @returns {Promise<ParseResult>}
 */
export async function parseMB51(file, fecha, almacen, user) {
  ...
  try {
    ...
    await logUpload(tipoLog, records.length, fecha, user)
    return { success: true, registros: records.length, fechaDatos: fecha }
  } catch (e) {
    return { success: false, registros: 0, error: e?.message || String(e) }
  }
...
export async function parseConciliacionEnvase() {
  return { success: false, registros: 0, error: 'Parser pendiente de implementación' }
}
export async function parseNivelServicio() {
  return { success: false, registros: 0, error: 'Parser pendiente de implementación' }
}
export async function parseIngresoEnvase() {
  return { success: false, registros: 0, error: 'Parser pendiente de implementación' }
}
export async function parseTiemposCarga() {
  return { success: false, registros: 0, error: 'Parser pendiente de implementación' }
}
 ...
 * @returns {Promise<ParseResult>}
 */
export async function runParser(tipo, file, fecha, user) {
  switch (tipo) {
    case 'inventario_liquido':
      return parseInventarioLiquido(file, fecha, user)
    case 'inventario_envase':
      return parseInventarioEnvase(file, fecha, user)
    case 'conteo_fisico':
      return parseConteoFisico(file, fecha, user)
    case 'salidas_rutas':
    case 'salidas':
      return parseSalidasRutas(file, fecha, user)
    case 'mb51_2000':
      return parseMB51(file, fecha, '2000', user)
    case 'mb51_2010':
      return parseMB51(file, fecha, '2010', user)
    case 'conciliacion_envase':
      return parseConciliacionEnvase()
    case 'nivel_servicio':
      return parseNivelServicio()
    case 'ingreso_envase':
      return parseIngresoEnvase()
    case 'tiempos_carga':
      return parseTiemposCarga()
    default:
